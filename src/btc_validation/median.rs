...
mod tests {
    ...
    use bellpepper_nonnative::util::{scalar::Fr, num};
    ...
    #[test]
    fn test_less_than_245bits() {
        let mut cs = TestConstraintSystem::<Fr>::new();

        let c = AllocatedNum::alloc(cs.namespace(|| "c"), || {
            let c_scalar = Fr::from_str_vartime("18283544428642297129396529020735695233361821945456783020785813").unwrap(); // block 123455
            Ok(c_scalar)
        }).unwrap();
        let d = AllocatedNum::alloc(cs.namespace(|| "d"), || {
            let d_scalar = Fr::from_str_vartime("171262555713783851185422181139260521316022447660158187451973632").unwrap();
            Ok(d_scalar)
        }).unwrap();
        let n_bits: usize = 250;
        let r2 = less_than(cs.namespace(|| "r2"), &c, &d, n_bits).unwrap().get_value().unwrap();
